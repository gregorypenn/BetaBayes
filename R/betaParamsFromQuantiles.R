...
#' @param q A vector of two quantiles (locations).
#' @param p A vector of two probabilities.
#' @param precision Desired precision in beta distribution parameters. Determines when iterative process stops.
#' @param derivative.epsilon Small value used for ε in the approximation of the derivatives of a few functions in the algorithm.
#' @param start.with.normal.approx Logical. Whether or not to start the iterative process with values obtained from a normal approximation to the Beta distribution.
#' @param start Vector (of length 2) of initial values for Beta distribution parameters. Ignored when start.with.normal.approx = TRUE.
#' @param report.details If FALSE then only beta parameters a and b are returned. See details for values returned if TRUE.
#' @param plot Logical. Whether or not to plot the resulting Beta distribution, along with tail probabilities obtained.
#' @return A list including at least a and b beta shape parameters. See details for more.
#' @details
#'
...
#'
#' $last.change
#' : Last change in $a and $b in the interative algorithm.
#'
#' $niter
...
#'
#' $p.check
#' : A quick validation: values for $a and $b returned lead to cumulative probabilities $p.check at the quantiles $q.
#'
#'
...
betaParamsFromQuantiles <- function(q,
                                    p = c(0.025,0.975),
                                    precision = 0.001,
                                    ...
                                    plot = FALSE)
{
  ...
  if (report.details) {
    l <- list(a=parms$theta[1], b=parms$theta[2], last.change=parms$last.change, niter=parms$niter, q=q, p=p, p.check=p.check)
  } else {
    l <- list(a=parms$theta[1], b=parms$theta[2])
  }
  return(l)
